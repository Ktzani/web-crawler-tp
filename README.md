...
│   ├── core/
│   │   └── frontier.py      # fila de URLs com politeness por host + log visited.txt
│   ├── network/
...
├── scripts/                 # utilitarios fora do caminho do crawl
│   ├── check_seeds.py       # classifica seeds em visitadas/redirecionadas/perdidas
│   ├── characterize.py      # remove URLs duplicadas do corpus (mantem 1a ocorrencia)   
│   ├── dedupe_corpus.py     # Caracterização do corpus
│   ├── speedup_experiment.py          # varia NUM_THREADS e mede tempo
│   └── extract_corpus_validation.py   # WARC → arquivos .html individuais
